...
- **HIGH — operator decision:** T-11 keeps the original entity split. Done means the agreed Task work is verified complete and ready for the operator delivery decision. Merged means the operator accepted and landed the Change. Feedback against existing acceptance criteria reopens the same Task; changed or additional intent is separately authorized work.
...
No gap affects T-11. A GitHub-native Issue or Project could own merge-coupled status, but the operator explicitly retained Backlog as the Task surface.
