...
Rewrite grilling as the engagement-first alignment contract — current-state questions before any recommendation or narrowing, boundaries engaged before proposals, plan built in conversation, operator corrections binding and restated, exactly one answered question card on the owning Task carrying the dependency inventory and reconciled role-map enumeration, one confirmation per unspecified field, one visible acknowledgment sentence before the next tool call, read-only until approval, generic assent never a disposition — and land the standing conduct rules as prose in the owning skills. Approved plan: doc-123.
