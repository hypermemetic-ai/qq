...
bin/qq-wave's rollback trap runs 'git branch -D' and 'git worktree remove --force'. The git rail blocks exactly those forms when an agent types them, but the rail is a Claude Code PreToolUse hook and never sees a git call made from inside a script — so qq's own tool performs, unattended, the operation the methodology forbids by hand. On 2026-07-09 an independent review reproduced that trap destroying a live worker's branch and files (fixed in 1070a83 by disarming the rollback once the agent starts), but the underlying tension stands: the rollback still force-deletes on its remaining paths. Decide whether an automated rollback of a seconds-old, never-pushed claim branch is a sanctioned exception (and say so in the methodology), or whether it must use a non-destructive form. Related: unlanded branches today still need an operator to delete, because the rail blocks the agent.
