...
Read AGENTS.md, CONCEPTS.md, skills/align/SKILL.md, skills/deliver-change/SKILL.md, the exact typed handoff, and its cited artifacts. Run the align contract for the whole confirmed batch. Obtain explicit operator approval before Repository mutation. Create the mapped Task record(s), approved plan(s), decision ledger(s), and Change(s) through Backlog's CLI from the Change worktree — records land in the operator store; never hand-edit managed markdown. One Task may cover multiple routed decisions and one routed decision may map to multiple Tasks, but every routed decision must be mapped.
