...
function withWriterLock(paths, callback) {
	...
	try {
		...
		if (error && error.code === "EEXIST") {
			fail(`trial writer lock is already held: ${paths.lock}`);
		}
		throw error;
	}
	let lockStat;
...
const DEACTIVATED_KEYS = new Set([...COMMON, "reason"]);
...
function validateDeactivated(record) {
...
function readWriter(paths) {
	try {
		lstatSync(paths.writer);
	} catch (error) {
		if (isMissing(error)) return null;
		throw error;
	}
	safeDirectory(paths.root);
	const writer = parseJson(readPrivateLeaf(paths.writer), paths.writer);
	if (!writer || typeof writer !== "object" || Array.isArray(writer)) fail("writer claim must be an object");
	exactKeys(writer, new Set(["schema", "owner", "pid", "claimed_at"]), "writer claim");
	if (writer.schema !== "qq.pi-code-tool-trial-writer/v1") fail("writer claim schema mismatch");
	requireString(writer, "owner", "writer claim");
	requireInteger(writer, "pid", "writer claim");
	if (writer.pid < 1) fail("writer claim.pid must be positive");
	validateTimestamp(writer.claimed_at, "writer claim.claimed_at");
	return writer;
}
...
export function claimWriter(paths, owner) {
	...
	try {
		writePrivateLeafExclusive(paths.writer, `${JSON.stringify({
			schema: "qq.pi-code-tool-trial-writer/v1",
			owner,
			pid: process.pid,
...
export function unlockWriter(paths, { isProcessAlive = processIsAlive } = {}) {
	const writer = readWriter(paths);
	if (!writer) fail("trial writer claim is missing");
	if (isProcessAlive(writer.pid)) {
		fail(`trial writer process ${writer.pid} is still alive; refusing unlock`);
	}
	unlinkSync(paths.writer);
	return writer;
}
...
export function deactivateTrial(paths, reason = "operator_stop") {
	return withWriterLock(paths, () => {
		...
		if (evidence.deactivated) fail("trial is already deactivated");
		const at = new Date().toISOString();
		appendRecordUnlocked(paths, {
			schema: LEDGER_SCHEMA,
			event: "deactivated",
...
function armSummary(arm, assignments, outcomes) {
	...
	return {
		...
		median_uncached_input_tokens: median(result.map((outcome) => outcome.usage_input)),
...
export function trialStatus(paths) {
	...
	try {
		lock_present = lstatSync(paths.lock) !== undefined;
	} catch (error) {
		if (!isMissing(error)) throw error;
	}
	return {
	...
	try {
		switch (command) {
			...
			case "deactivate":
				...
				return 0;
			case "unlock": {
				const writer = unlockWriter(paths);
				console.log(`removed stale trial writer claim for pid ${writer.pid}`);
				return 0;
			}
			case "analyze":
