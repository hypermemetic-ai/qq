...
description: Practical map of QQ's operator staging, transcript and dictation privacy, Backlog write guard, Grok paraphrase-stall recovery, and continue shortcut.
tags: [pi-extension, safety, privacy, herdr]
openwiki:
  roles: [architecture, workflow, testing]
  change_kinds: [lifecycle, safety]
  source_paths: [extensions/operator-stage.ts, extensions/session-scrub.ts, extensions/dictation-private.ts, extensions/backlog-guard.ts, extensions/grok-paraphrase-guard.ts, extensions/continue.ts]
  symbols: [registerGrokParaphraseGuard, applyFallbackProfile]
  test_paths: [tests/test-operator-stage.mjs, tests/test-session-scrub.mjs, tests/test-dictation-private.mjs, tests/test-backlog-guard.mjs, tests/test-grok-paraphrase-guard.mjs, tests/test-continue.mjs]
...
| Grok paraphrase guard | On `grok-4.6` only, five adjacent text-bearing turns with trigram Jaccard similarity at least 0.6 abort. The first trip rewinds without summarization to the last usable non-user leaf; a second trip switches through [execution profiles](execution-profiles.md) to runner `sol-high`, or stops if that configured model/auth is unavailable. Short/tool-only turns do not add text; model or tree changes reset the streak. | `tests/test-grok-paraphrase-guard.mjs` |
...
flowchart TD
    ...
    Turn["Grok assistant turn ends"] --> Repeat{"Fifth similar text turn"}
    Repeat -->|first trip| Rewind["Abort and rewind once"]
    Repeat -->|later trip| Fallback["Switch to runner sol-high or stop"]
```

*Destructive work is fenced to finalized, identity-matched state; Grok recovery rewinds once before changing profile.*
...
For Grok recovery changes, test similarity boundaries, short/tool-only turns, non-Grok isolation, first rewind, second-trip fallback success/failure, profile event emission, and session/tree resets in `tests/test-grok-paraphrase-guard.mjs`. Run the matching `node --experimental-strip-types tests/test-<name>.mjs .`; use `npm test` only when registration order or shared lifecycle behavior changes.