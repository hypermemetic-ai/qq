...
The plugin provides `qq-relay` and registers `relay_list`, `relay_send`, and `relay_status` when DSH tools are available. Canonical addresses are live `session-<UUID>` IDs; short spoken aliases come from the [`qq` service](../runtime/dsh-console.md#projects-and-files), not a relay-owned map. If qq is absent, ID addressing still works and aliases disappear.
