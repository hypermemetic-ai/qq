...
| Refresh OpenWiki safely or change its shell-check environment | [OpenWiki automation](operations/runbook.md#openwiki-automation) | `bin/qq-openwiki-refresh`; `bin/qq-openwiki-shell-env.cjs`; `systemd/user/qq-openwiki.*` | isolated worktree, output allowlist, safe shell environment, locked merge | `tests/test-openwiki-refresh.sh` | `tests/test-openwiki-refresh.sh` |
