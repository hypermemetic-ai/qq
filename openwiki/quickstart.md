...
| Start, inspect, back up, or test | [Operations](operations/runbook.md) | `bin/event-plane*`; `package.json` | `serve`, `backup`, `shutdown` | relevant focused suite | inspect first; use `npm test` only for cross-system changes |
| Refresh OpenWiki safely | [OpenWiki automation](operations/runbook.md#openwiki-automation) | `bin/qq-openwiki-refresh`; `systemd/user/qq-openwiki.*` | isolated worktree, output allowlist, locked merge | `tests/test-openwiki-refresh.sh` | `tests/test-openwiki-refresh.sh` |
