...
The merge-gating Check loop is `tools/test.sh`: it runs every top-level `tests/test-*.sh`, and GitHub Actions invokes the same script for pull requests and pushes to `main`. CI installs Node `22.22.3` and stock Pi `0.81.1` with lifecycle scripts disabled for extension checks; the Repository has no root dependency-install step.
...
| Stock Pi entrypoint | `bash tests/test-qq-pi-runtime.sh` | Missing npm/global package/CLI, changed cwd or environment, argument-forwarding drift |
| Execution policy and delegation | `bash tests/test-qq-execution-profiles.sh`; `bash tests/test-qq-delegate.sh`; `bash tests/test-qq-service-class-extension.sh` | Wrong role/provider/model/effort, malformed or unsupported service-class payloads, non-private or reused run dirs, missing envelopes, malformed `TERMINAL`, inherited Context7 credentials |
| Mounted Pi extensions | `bash tests/test-qq-extension-mount.sh` plus the owning `tests/test-qq-*-extension.sh`; for operator interaction, include `test-qq-communication-moments-extension.sh`, `test-qq-delegate-watch-extension.sh`, and `test-qq-footer-extension.sh` | Imported but uninvoked extensions, accidental activation in delegates/headless runs, duplicate completion wakes, unsafe run records, argument coercion, stale dependencies |
...
- Focused harnesses cover the stock Pi entrypoint, role and service-class policy, mounted extensions, delegate/Change/board/handoff/Observer/reaper engines, OpenWiki daily/merge guards, the Pi Backlog drift-net, and Herdr adapters.
