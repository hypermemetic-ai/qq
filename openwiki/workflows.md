...
For genuinely new work, the operator-facing accountable owner uses the default alignment brief: current-state questions first, every embedded consequential decision cited or recommended, one answered question card, and one closing approval question. The mounted communication-moments extension injects its doctrine on the first turn of an interactive operator-facing session and provides `operator_ask` for batched alignment, realignment, operator-action, and judgment-reserved-delivery questions. It is deliberately absent from delegated runs and `--print`/`-p` headless sessions. Before Repository mutation, the owning Task's decision ledger must cite what settled every consequential decision or explicitly say `none`; dispositions do not transfer. Spawned, delegated, review, research, maintainer, and event-triggered Actors treat bounded assignments as aligned and return new consequential decisions or scope gaps to the assigning or owning Actor (`CONCEPTS.md`; `extensions/qq-communication-doctrine.md`; `skills/deliver-change/SKILL.md`).
...
Invoke the assigned worktree's resident `qq-delegate run` or `batch`. It resolves that checkout's stock Pi wrapper, role manifest, timeout, tools, and execution-profile policy; calls block through child lifecycle. A nonzero exit or missing `ENVELOPE.md` fails dispatch. On infrastructure failure, resume once with the same brief in a fresh run directory; a second failure is `inconclusive-under-substrate`. The owner verifies every envelope claim against the tree. While a run lacks `TERMINAL`, the mounted watcher shows it beneath the accountable footer (up to eight rows plus overflow). When `TERMINAL` first appears, one session claims the completion and sends the exit/timeout summary to the spawning Herdr pane recorded by `qq-delegate`; if that pane has no unique live intercom presence, it falls back to an operator Herdr notification. Existing completions are not replayed at session start. This visibility does not replace the durable artifacts as delegate evidence (`skills/delegate-batch/SKILL.md`; `bin/qq-delegate`; `extensions/qq-delegate-watch.ts`).
