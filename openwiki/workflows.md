...
6. If merged, verify the merge commit is on fresh `origin/main` and fast-forward the single clean registered `main` checkout to a frozen target OID. At terminal disposition, keep the entire work session and checkout intact for inspection, then focus the validated home board without moving the accountable pane; only the operator retires the session later. If closed, report and apply the unmet-criterion or changed-intent rule. If still open after the window, report its URL and Checks.

Fresh-context review remains after implementation and local verification but before the first commit or publication. The detailed refusal rules for browser visibility, primary-main synchronization, and terminal work-session disposition—including preserving the complete session and checkout for inspection and reserving retirement for the operator—are canonical in `skills/deliver-change/SKILL.md:11-132`.
