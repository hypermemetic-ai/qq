...
For durable messages it sends kind `agent.message` to recipient `agents/<canonical-session-id>`. A background recipient consumer uses generation `0` and a fresh endpoint token. Unsupported payloads are blocked. Valid messages are injected into Pi; acknowledgement happens only after the session transcript contains the matching event ID and content hash. If persistence is not observable, the adapter retries instead. Immediate delivery may abort a busy turn, with an idempotent `agent.immediate-claim` publication coordinating the interrupt; injection waits up to five seconds for Pi to become idle and retries if it does not. Presence starts and renews with the Pi session, reflects thinking/tool/idle events, and is removed on shutdown. See [Profiles and extensions](../runtime/profiles-and-extensions.md) for its Pi-facing behavior.

[Delegation and review](../runtime/delegation-and-review.md) also uses recipient obligations for worker outcomes. `bin/lib/run-events.mjs` sends `run.blocked` from `qq/review-worker` and `run.landed` from `qq/land-worker` to `qq/review-flow/<architect-session>`, using deterministic request hashes. The architect-only receiver validates the product, producer/origin, recipient, session, and `qq.run-{blocked,landed}/v1` schema; it applies the same transcript-receipt-before-acknowledgement rule and blocks unsupported outcomes. These are dedicated outcome records, not `qq.agent-message/v2` payloads.
