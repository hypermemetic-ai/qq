...
10. **HITL bash script.** Last resort. If a human must click and the loop can run in a real interactive tty, drive _them_ with `scripts/hitl-loop.template.sh` so the loop is still structured. Captured output feeds back to you. Reserved operator handoffs are different: use the methodology's operator-handoff rules instead, with no `read` prompts.
...
- [ ] **Agent-runnable** — you can run it unattended; a human in the loop only via `scripts/hitl-loop.template.sh`, and only when that template has a real interactive tty.
