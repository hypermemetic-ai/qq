...
#
# hypercore rail: allows normal `git push` so agents can ship, but blocks the
...
# version blocked ALL pushes; hypercore narrows that to force-pushes only.
...
for pattern in "${DANGEROUS_PATTERNS[@]}"; do
  if echo "$COMMAND" | grep -qE "$pattern"; then
    echo "BLOCKED: '$COMMAND' matches destructive-git pattern '$pattern'. The hypercore rail allows normal 'git push' but blocks force-push, reset --hard, clean -f, branch -D, checkout/restore ., and history rewrites." >&2
    exit 2
  fi
done
