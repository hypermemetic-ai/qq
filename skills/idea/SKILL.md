---
name: idea
description: Captures an explicitly supplied idea in the repository-root ideas.md without interrupting the current task. Use only when the operator's message begins with "idea:" or explicitly invokes "$idea" with text.
...
3. Create root-level `ideas.md` with a `# Ideas` heading if it does not exist.
...
5. Reply `captured → ideas.md` in one line, then resume the interrupted task.
