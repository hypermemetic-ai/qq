...
const liveCss = httpApp.resolveAsset("console-v9.css", true);
assert.equal(liveCss.live, true);
assert.match(liveCss.body.toString("utf8"), /@font-face/);
const bakedCss = httpApp.resolveAsset("console-v9.css", false);
assert.equal(bakedCss.live, false);
assert.equal(httpApp.resolveAsset("sw-v10.js", true).live, false);
assert.equal(httpApp.internals.LIVE_ASSET_FILES["console-v9.css"], "assets/console.css");
...
try {
  const first = await fetch(`http://127.0.0.1:${livePort}/qq/assets/console-v9.css`);
  assert.equal(first.status, 200);
  assert.match(first.headers.get("cache-control"), /no-store/);
  assert.match(await first.text(), /@font-face/);
  await writeFile(cssPath, `${originalCss}\n/* design-loop-live-proof */\n`);
  const second = await fetch(`http://127.0.0.1:${livePort}/qq/assets/console-v9.css`);
...
try {
  const baked = await fetch(`http://127.0.0.1:${bakedServer.address().port}/qq/assets/console-v9.css`);
