...
assert.deepEqual(resolved.profile, { provider: "openai-codex", model: "gpt-5.6-sol", effort: "xhigh", serviceClass: "priority" });
...
assert.equal(callerUnsafe, undefined); assert.equal(callerRound, 25); assert.deepEqual(new Set(callerModels), new Set(["gpt-5.6-sol"]), "unsafe Sol evidence retried the Actor instead of falling directly to native Pi"); assert.ok(h.notifications.some((note) => /unsafe\/invalid.*native compaction/.test(note.message)));
