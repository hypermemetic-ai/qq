...
# Gitignored machine-local content must not be measured: operator machine and
# CI give identical answers.
cp "$fixture/.gitignore" "$tmp/gitignore-before-ignored-probe"
printf '%s\n' '--profile' >"$fixture/skills/ratchet-ignored-probe.txt"
printf '%s\n' \
  '/skills/ratchet-ignored-probe.txt' >>"$fixture/.gitignore"
fixture_track
"$fixture_ratchet" check >"$tmp/output"
rm "$fixture/skills/ratchet-ignored-probe.txt"
